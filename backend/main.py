...
API_BASE_URL    = os.environ.get("API_BASE_URL", "https://sistemasn-production.up.railway.app")
...
def cadastro(req: CadastroRequest):
    ...
    if existente.data:
        raise HTTPException(400, "Este email já possui uma conta. Use sua chave de acesso.")
...
def validar_licenca(req: ValidarRequest):
    ...
    if lic["expira_em"]:
        ...
        if datetime.now(timezone.utc) > expira:
            ...
            if lic["plano"] == "trial":
                db.table("licencas").update({"plano": "suspenso"}).eq("chave", req.chave).execute()
...
def gerar_pagamento(req: GerarPagamentoRequest):
    ...
    resp = httpx.post(f"{MP_API}/v1/payments", json=payload, headers=headers, timeout=15)
...
async def webhook_pagamento(request: Request):
    ...
    resp = httpx.get(f"{MP_API}/v1/payments/{mp_payment_id}", headers=headers, timeout=10)
    ...
    pag = db.table("pagamentos").select("*").eq("mp_payment_id", int(mp_payment_id)).execute()
    if not pag.data:
        return {"ok": True}
...
def gerar_licenca(req: GerarLicencaRequest):
    ...
    return {"chave": chave, "nome_cliente": req.nome_cliente,
            "email": req.email, "expira_em": expira_em}
...
def remover_maquina(chave: str, machine_id: str):
    ...
    if not res.data:
        raise HTTPException(404, "Chave não encontrada.")
    db.table("maquinas").delete().eq("licenca_id", res.data[0]["id"]).eq("machine_id", machine_id).execute()
